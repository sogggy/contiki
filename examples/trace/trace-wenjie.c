...
#define SLOT_TIME RTIMER_SECOND/40    // 80 HZ, 0.0125s
#define SLOT_DIM 15
...
static void insert(discovery_struct *data) {
  if (node_count >= ARRAY_SIZE) {
    return;
  }

  int index = hash_code(data->node_id);

  while (hash_table[index].node_id != -1) {
    index = (index + 1) % ARRAY_SIZE;
  }
  memcpy(&hash_table[index], data, sizeof(discovery_struct));
  node_count++;
  printf("%3lu: New node %lu inserted at %d\n", curr_timestamp / CLOCK_SECOND, data->node_id, index);
...
broadcast_recv(struct broadcast_conn *c, const linkaddr_t *from)
{
  ...
  printf("Received packet from node %lu with strength %d\n", received_packet.src_id, (signed short) packetbuf_addr(PACKETBUF_ATTR_RSSI));
  bool is_in_proximity = true;
  if ((signed short) packetbuf_addr(PACKETBUF_ATTR_RSSI) < RSSI) {
