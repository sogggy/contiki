...
char sender_scheduler(struct rtimer *t, void *ptr) {
  ...
  static int row_wake = 0;
  static int col_wake = 0;
  PT_BEGIN(&pt);

  curr_timestamp = clock_time(); 
  printf("Start clock %lu ticks, timestamp %3lu.%03lu\n", curr_timestamp, curr_timestamp / CLOCK_SECOND, ((curr_timestamp % CLOCK_SECOND)*1000) / CLOCK_SECOND);
  printf("Radio will wake along row %i and col %i", row_wake, col_wake);
  while(1){
    row_wake = random_rand() % (ROWS);
    col_wake = random_rand() % (COLS);
    // radio on
    NETSTACK_RADIO.on();
    for (i = 0; i < ROWS * COLS; i++) {
      if (
        (i >= row_wake * 10 && i < (row_wake + 1) * 10) // same row
        || (i % 10 == col_wake) //same col
      ) {
        //send
        printf("i is %i and radio is sending", i);
